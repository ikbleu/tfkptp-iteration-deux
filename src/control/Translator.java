...
public int toKeyCode(String keyRepresentation)
{
	return 0;
}

//Taken from 
//http://www.java2s.com/Code/Java/Event/ConvertingaKeyStroketoaString.htm
public static String toString(KeyEvent key) {
    int m = key.getModifiers();
    int keyCode = key.getKeyCode();
    String toReturn="";

    if ((m & (KeyEvent.SHIFT_DOWN_MASK | KeyEvent.SHIFT_MASK)) != 0) {
      toReturn.concat("SHIFT+");
    }
    if ((m & (KeyEvent.CTRL_DOWN_MASK | KeyEvent.CTRL_MASK)) != 0) {
      toReturn.concat("CTRL+");
    }
    if ((m & (KeyEvent.ALT_DOWN_MASK | KeyEvent.ALT_MASK)) != 0) {
      toReturn.concat("ALT+");
    }
    if (keyCode >= KeyEvent.VK_NUMPAD0 && keyCode <= KeyEvent.VK_NUMPAD9) 
        toReturn.concat("NUMPAD+");
    
    toReturn.concat(KeyEvent.getKeyText(keyCode));
    return toReturn;
  }
